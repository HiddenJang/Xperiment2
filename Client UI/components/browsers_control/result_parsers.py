...
class SeleniumParser(QObject):
    ...
    def start(self) -> dict | None:
        ...
        self.driver.quit()
        if not results:
            self.finish_signal.emit({'status': 'Не удалось получить результаты событий по ранее сделанным ставкам (Selenium)',
                                     'results': {}})
        elif len(results) != len(self.active_bets_data):
            self.finish_signal.emit({'status': 'Результаты событий получены не по всем ранее сделанным ставкам (Selenium)',
                                     'results': results})
        else:
            self.finish_signal.emit({'status': 'Результаты событий получены по всем ранее сделанным ставкам (Selenium)',
                                     'results': results})
...
class ApiResponseParser(QObject):
    ...
    async def start_async(self) -> dict:
        ...
        if not results:
            ...
            self.finish_signal.emit({'status': 'Результаты событий получены не по всем ранее сделанным ставкам (API)',
                                     'results': results})
        else:
            self.finish_signal.emit({'status': 'Результаты событий получены по всем ранее сделанным ставкам (API)',
                                     'results': results})
